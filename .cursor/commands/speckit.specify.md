...
**在执行任何其他操作之前**, 你必须首先加载项目描述文件以了解项目背景:

0. **加载项目描述**: 读取 `.specify/memory/project-description.md` 文件(如果存在), 以了解:
   - 项目概述和目标
   - 目标用户群体
   - 核心功能领域
   - 项目约束与假设
   
   在创建功能规范时, 确保新功能与项目整体目标和约束保持一致. 使用项目描述中的信息来:
   - 理解功能在项目整体上下文中的位置
   - 做出符合项目目标的假设和决策
   - 识别功能与项目其他部分的潜在依赖关系

0.5. **前置条件检查**: 在执行任何功能规范创建之前, 必须检查以下必需的设计文档是否存在:
   
   - 检查 `.specify/memory/model-design.md` 文件是否存在
   - 检查 `.specify/memory/ocr_voice_solution.md` 文件是否存在
   
   **如果任一文件不存在**:
   - 立即停止执行
   - 返回错误消息: "前置条件未满足: 缺少必需的设计文档. 请先完成以下文档: [列出缺失的文件], 然后重试."
   - 不执行后续任何步骤(包括分支创建、规范编写等)
